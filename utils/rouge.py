...
def write_summaries(path, hyp, ref):
    import os
    sys_path = path + "/system_summaries"
    model_path = path + "/model_summaries"
    if not os.path.exists(sys_path):
        os.makedirs(sys_path)
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    for i, (h, r) in enumerate(zip(hyp, ref)):
        with open(sys_path+"/sys.{}.txt".format(i), "w") as f:
            f.write(r+"\n")
        with open(model_path+"/model.A.{}.txt".format(i), "w") as f:
            f.write(h+"\n")

def rouge_official(path, hyp, ref):
    from pyrouge import Rouge155
   
    write_summaries(path, hyp, ref) 
    r = Rouge155()
    r.system_dir = path+'/system_summaries'
    r.model_dir = path+'/model_summaries'
